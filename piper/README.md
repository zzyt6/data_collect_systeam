...
conda run -n datacollect python piper/tools/diffusion_xy_collector.py \
  --list-cameras \
...
```bash
conda run -n datacollect python piper/tools/initial_pose_tuner.py \
  --can can0 \
  --camera-wrist 10 \
  --camera-global 4 \
  --connect \
...
```text
piper/initial_pose/initial_pose.json
```
...
```text
piper/docs/piper_diffusion_xy_schema.md
```
...
- `data/`
- `piper/initial_pose/*.json`
- `__pycache__/`
- 日志、临时文件、视频和压缩包

不要把真机采集数据、私有初始姿态和本地设备路径直接提交到公开仓库。需要发布数据时，建议单独做数据集 release、使用对象存储，或上传到 Hugging Face Dataset。

当前仓库把 `piper/piper_sdk/` 和 `piper/Piper_sdk_ui/` 作为 Git submodule 管理。克隆后运行
